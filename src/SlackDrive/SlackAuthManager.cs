...
public class SlackAuthManager
{
    ...
    private string AuthorizeWithOAuth(CancellationToken ct)
    {
        ...
        string scopes = _settings.Scopes
            ?? "channels:read channels:history groups:read groups:history im:read im:history mpim:read mpim:history users:read chat:write search:read.public search:read.private search:read.im search:read.mpim search:read.files search:read.users";
